...
def tp_index(event_tracer, tp_info, project_dock, result_path, start_time, finish_time):  # get get tp moving distance and time each day
    ...
    capacity_2.sort()
    tp_output = dict()
    for capacity in capacity_1:
        tp_output["{0}_1".format(capacity)] = dict()
        tp_output["{0}_1".format(capacity)] = {time : 0.0 for time in time_list}
    for capacity in capacity_2:
        tp_output["{0}_2".format(capacity)] = dict()
        tp_output["{0}_2".format(capacity)] = {time: 0.0 for time in time_list}
    date_time_list = list()
    for date_time in time_list:
        ...
        for idx in range(len(daily_event)):
            ...
            tp_output["{0}_{1}".format(available_tp, yard)][date_time] += temp['Distance']

    yard_tp = pd.DataFrame.from_dict(tp_output)
    yard_tp["Date"] = date_time_list
...
def calculate_occupied_area(result_path, event_tracer, factory_info, start_date, finish_date):
    ...
    event_tracer = event_tracer[
        (event_tracer['Event'] == "Process In") | (event_tracer['Event'] == "Process Out") | (
                    event_tracer['Event'] == "Stock In") | (event_tracer['Event'] == "Stock Out")]
        ...
        if block_code in block_info.keys():
            ...
            if len(moving_event) > 0:
                ...
                if len(moving_distance) > 0:
                    avg_distance = np.sum(moving_distance)
